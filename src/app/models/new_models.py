...
meeting_coordinator_association = Table(
    'meeting_coordinator_association', Base.metadata,
    Column('meeting_id', Integer, ForeignKey('meetings.id'), primary_key=True),
    Column('employee_id', Integer, ForeignKey('employees.id'), primary_key=True)
)

meeting_moderator_association = Table(
    'meeting_moderator_association', Base.metadata,
    Column('meeting_id', Integer, ForeignKey('meetings.id'), primary_key=True),
    Column('employee_id', Integer, ForeignKey('employees.id'), primary_key=True)
)
...
class Meeting(BaseTable):
    ...
    coordinators = relationship("Employee", secondary=meeting_coordinator_association, back_populates="coordinated_meetings")
    moderators = relationship("Employee", secondary=meeting_moderator_association, back_populates="moderated_meetings")
    zoom_meeting_id = Column(String(255), nullable=True)  # Zoom Meeting ID for virtual meetings
    zoom_participant_report = Column(Text, nullable=True)  # JSON data of participant report from Zoom
...
class Trip(BaseTable):
    ...
    vehicle_id = Column(Integer, ForeignKey('vehicles.id'))
    driver_id = Column(Integer, ForeignKey('drivers.id'))
    trip_start_location_id = Column(Integer, ForeignKey('trip_start_locations.id'), nullable=True)
...
class TripStage(BaseTable):
    ...
    mileage_at_arrival = Column(DECIMAL(10, 2), nullable=False)
...
class Employee(BaseTable):
    ...
    created_timesheets = relationship("TimeSheet", foreign_keys="[TimeSheet.created_by_id]", back_populates="created_by")
    checked_timesheets = relationship("TimeSheet", foreign_keys="[TimeSheet.checked_by_id]", back_populates="checked_by")
    approved_timesheets = relationship("TimeSheet", foreign_keys="[TimeSheet.approved_by_id]", back_populates="approved_by")
    ...
    is_internal_auditor = Column(Boolean, nullable=True)  # New field
    is_compliance = Column(Boolean, nullable=True)  # New field
    is_finance = Column(Boolean, nullable=True)  # New field
    is_hr = Column(Boolean, nullable=True)  # New field
...
class PerformanceReview(BaseTable):
    ...
    reviewer = relationship("Employee", foreign_keys=[reviewer_id], back_populates="performance_reviews_given")
...
class Project(BaseTable):
    ...
    work_plans = relationship("WorkPlan", back_populates="project")  # New relationship
...
class Funding(BaseTable):
    ...
    component_id = Column(Integer, ForeignKey('project_components.id'), primary_key=True)
...
class TimeSheet(BaseTable):
    ...
    employee = relationship("Employee", foreign_keys="[TimeSheet.employee_id]", back_populates="timesheets")
    project = relationship("Project", back_populates="timesheets")
    created_by = relationship("Employee", foreign_keys="[TimeSheet.created_by_id]", back_populates="created_timesheets")
    checked_by = relationship("Employee", foreign_keys="[TimeSheet.checked_by_id]", back_populates="checked_timesheets")
    approved_by = relationship("Employee", foreign_keys="[TimeSheet.approved_by_id]", back_populates="approved_timesheets")
    archived_by = relationship("Employee", foreign_keys="[TimeSheet.archived_by_id]", back_populates="archived_timesheets")

class TripStartLocation(BaseTable):
    __tablename__ = 'trip_start_locations'
    name = Column(String(255), nullable=False)
    description = Column(Text, nullable=True)
    latitude = Column(DECIMAL(9, 6), nullable=True)
    longitude = Column(DECIMAL(9, 6), nullable=True)
    tenancy_id = Column(Integer, ForeignKey('tenancy.id'))
    tenancy = relationship("Tenancy", back_populates="start_locations")
    trips = relationship("Trip", back_populates="trip_start_location")
